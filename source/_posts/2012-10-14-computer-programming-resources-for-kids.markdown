...
A generation ago, beginning programmers had access to very simple programming environments like BASIC and Logo.  You could turn on an Apple II computer without a disk or an operating system and it would boot right into BASIC.  Programming was just part of using a computer, and anybody who used a computer dabbled in programming.  Now there are many layers between the user and the programming environment, which has caused computer users to split into two different groups: users, and programmers.  Now most people who use computers don't program because they don't have to.  And so programming isn't as accessible as it used to be.  The infamous [Why The lucky stiff](http://en.wikipedia.org/wiki/Why_the_lucky_stiff) famously referred to this problem as [The Little Coder's Predicament](https://github.com/hacketyhack/hacketyhack/wiki/The-Little-Coder's-Predicament).
